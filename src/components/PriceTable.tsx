    ...
    <section id="pricing" className="py-20 bg-[#0a0a0a]">
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="grid lg:grid-cols-2 gap-8 items-stretch">
          {/* Photo */}
          <div className="relative rounded-3xl overflow-hidden min-h-[300px] order-2 lg:order-1">
            <img
              src="/works/work2.jpg"
              alt="Свайно-винтовой фундамент под ключ"
              className="absolute inset-0 w-full h-full object-cover"
              loading="lazy"
            />
            <div className="absolute inset-0 bg-gradient-to-t from-[#0a0a0a] via-[#0a0a0a]/20 to-transparent" />
            <div className="absolute bottom-0 left-0 p-6">
              <p className="text-white font-black text-xl mb-1">Фундамент под ключ</p>
              <p className="text-brand-orange text-sm font-bold tracking-widest uppercase">от 35 000 ₽</p>
            </div>
          </div>

          {/* Table + button */}
          <div className="order-1 lg:order-2 flex flex-col">
            <div ref={ref} className="liquid-glass rounded-3xl overflow-hidden flex-1">
              <div className="grid grid-cols-3 bg-white/5 px-6 py-4">
                ...
                <p className="text-white/40 text-[10px] tracking-[0.2em] uppercase font-semibold text-right">Цена</p>
              </div>
              ...
              {pricing.rows.map((row, i) => (
                ...
                >
                  ...
                  <p className="text-white/50 text-xs">{row.description}</p>
                  <p className="text-brand-orange font-black text-base text-right">{row.price}</p>
                </div>
              ))}
            </div>
            ...
            <a
              href="#contact"
              className="mt-6 text-center bg-brand-orange hover:bg-orange-400 text-black font-bold text-sm tracking-[0.2em] uppercase px-10 py-4 rounded-full transition-all duration-300 hover:scale-[1.02] shadow-lg shadow-brand-orange/20"
            >
